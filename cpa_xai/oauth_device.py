...
def _post_form_curl(
    ...
    *,
    ...
    """curl_cffi path — supports socks5 / socks5h (stdlib urllib does not)."""
...
def _post_form(
    ...
    *,
    ...
    # Prefer curl_cffi for SOCKS (urllib: "unknown url type: socks5h") and
    # generally for residential TLS stability.
    p = resolve_proxy(proxy)
    use_curl = True
    try:
        from curl_cffi import requests as _cf  # noqa: F401
    except ImportError:
        use_curl = False
    if use_curl and (not p or _proxy_is_socks(p) or True):
        ...
        for i in range(attempts):
            try:
                ...
                last = e
                # HTTP status errors are returned as (status, body) above, not raised
                if not _is_transient_net_error(e) and "Failed to perform" not in str(e):
                    # still retry network-ish curl errors
                    msg = str(e).lower()
                    if not any(
                        k in msg
                        for k in (
    ...
    # stdlib fallback (HTTP proxies only — no socks5h)
