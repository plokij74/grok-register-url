...
def mint_with_sso_protocol(
    *,
    ...
    # 1) Validate SSO (chrome131 matches register path; soft-retry once on reset)
    ...
    for attempt in range(2):
        try:
            ...
            last_net = e
            log(f"protocol accounts.x.ai blip {attempt + 1}/2: {e}")
            if attempt == 0:
                import time as _time

                _time.sleep(1.0)
                continue
    if last_net is not None or r is None:
        raise ProtocolMintError(
            f"accounts.x.ai network error: {last_net}"
        ) from last_net
