...
def create_standalone_page(
    *,
    ...
    # Prefer residential helper: supports user:pass via temp Chrome extension
    applied = False
    if proxy:
        try:
            import residential_proxy as rp  # type: ignore

            applied = bool(
                rp.apply_proxy_to_chromium_options(opts, proxy, log=log)
            )
        except Exception as exc:
            log(f"residential proxy apply failed: {exc}")
            applied = False
    if not applied:
...
def _wait_turnstile(
    ...
    *,
    ...
    while time.time() < deadline:
        # Cookie overlay can block the checkbox
        try:
        ...
        now = time.time()
        # Fast poll; only slow down right before actual click (helper does 4–8s)
        # After a click, give CF ~10s grace before re-click
        if click_count > 0 and (now - last_click_at) < 10.0:
            _sleep(0.5)
            continue
        if click_count >= 5:
            _sleep(0.5)
        ...
        # Prefer register-machine helpers when available (random delay inside click)
        try:
            import grok_register_ttk as reg  # type: ignore

            if hasattr(reg, "_click_turnstile_checkbox"):
                ok = reg._click_turnstile_checkbox(
                    page,
                ...
                if ok and not clicked:
                    log("clicked turnstile via register helper")
                    clicked = True
                _sleep(0.5)
                continue
        except Exception:
        ...
        # Fallback path: apply 4–8s delay only right before click
        delay = random.uniform(4.0, 8.0)
        log(f"turnstile click in ~{delay:.1f}s")
        _sleep(delay)
        last_click_at = time.time()
        click_count += 1

        # Fallback: shadow-root checkbox / iframe center click
        try:
            ...
            if iframe is None:
                try:
                    iframe = page.ele('css:iframe[src*="challenges.cloudflare.com"]', timeout=0.3)
                except Exception:
                    iframe = None
  ...
  Object.defineProperty(MouseEvent.prototype, 'screenY', { get: function(){ return sy; }, configurable: true });
                ...
                try:
                    ...
                    try:
                        iframe.click()
                        if not clicked:
                            log("clicked turnstile iframe fallback")
                            clicked = True
                    except Exception:
                        pass
        except Exception:
